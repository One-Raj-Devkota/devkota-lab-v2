<script lang="ts">
  ...
  onMount(() => {
    ...
    type Particle = { x:number; y:number; vx:number; vy:number; r:number; a:number; pulse:number; };
    ...
    const resize = () => {
      ...
      particles = Array.from({ length: count }, () => ({
        ...
        vx: (Math.random()-.5)*.16, vy:(Math.random()-.5)*.14,
        r:.4+Math.random()*1.5, a:.18+Math.random()*.55,
    ...
    const draw = () => {
      ...
      for (const p of particles) {
        ...
        const alpha = Math.max(.08, p.a + Math.sin(frame*1.8+p.pulse)*.12);
        ctx.beginPath();
        ...
        if (distance < 155) {
          ctx.beginPath();
          ctx.strokeStyle = `rgba(123,97,255,${.16*(1-distance/155)})`;
          ctx.lineWidth = .7;
          ctx.moveTo(p.x,p.y);
...
<main>
  <section class="hero" id="top">
    ...
    <div class="orb-stage reveal visible" aria-hidden="true">
      ...
      <div class="floating-card"><span class="quote">“</span><b>Technology</b><p>Connect ideas, people and possibilities.</p></div>
    </div>
  ...
  <section class="section" id="about">
    ...
    <div class="about-layout">
      ...
      <div class="value-grid reveal">
        <article data-tilt><i>✦</i><div><b>Problem Solver</b><span>課題を小さく分解して前に進める。</span></div></article>
        <article data-tilt><i>◎</i><div><b>Quality Mindset</b><span>テスト経験を活かして品質を見る。</span></div></article>
        <article data-tilt><i>⌘</i><div><b>Lifelong Learner</b><span>知らないことを学び続ける。</span></div></article>
        <article data-tilt><i>◉</i><div><b>Global Mindset</b><span>日本で働き、多様な視点を大切にする。</span></div></article>
      </div>
  ...
  <section class="section skills-section" id="skills">
    ...
    <p class="skill-note reveal">※ 数値はポートフォリオ上の視覚表現。技術ごとに実務経験の深さは異なります。</p>
  </section>
    ...
    <div class="projects-grid">
      {#each projects as project}
        <article class="project-card reveal" data-tilt>
          <div class="project-visual"><div class="project-orb"></div><div class="project-lines"></div><span>{project.number}</span></div>
  ...
  <section class="section" id="experience">
    ...
    <div class="journey reveal">
      ...
      <blockquote><span>“</span>Small steps,<br />better every day.<small>— Devkota Lab</small></blockquote>
    </div>
  </section>
...
<footer>
  <a class="brand footer-brand" href="#top"><span class="brand-orbit"><i></i></span><span><b>Devkota <em>Lab</em></b><small>BUILD · LEARN · CREATE · BEYOND</small></span></a>
...
.glass{border:1px solid rgba(128,159,225,.18);background:rgba(11,18,39,.6);backdrop-filter:blur(20px)}.metrics{grid-column:1/-1;display:grid;grid-template-columns:repeat(4,1fr);border-radius:16px;padding:20px 25px;margin-top:18px;box-shadow:inset 0 1px rgba(255,255,255,.025),0 18px 70px rgba(0,0,0,.18)}.metrics div{display:flex;flex-direction:column;gap:3px;padding:4px 25px;border-right:1px solid rgba(114,139,193,.14)}.metrics div:last-child{border-right:0}.metrics strong{font-size:25px;color:#f4f8ff}.metrics span{font-size:10px;color:#74819c;text-transform:uppercase;letter-spacing:.1em}
.section{width:min(1180px,90vw);margin:0 auto;padding:105px 0;position:relative;border-top:1px solid rgba(105,133,191,.12)}.section-label{color:#6f7c99;font-size:10px;letter-spacing:.19em;margin-bottom:22px}.section-label span{color:#55bdfd;margin-right:8px}.section h2{font-size:clamp(38px,5vw,65px);line-height:1.03;letter-spacing:-.05em;margin:0}.section h2 span{color:#9d72ff}.about-layout{display:grid;grid-template-columns:1.05fr 1fr .72fr;gap:22px;align-items:stretch}.about-copy{padding-right:18px}.about-copy .lead{font-size:18px;color:#d4d9e7;line-height:1.65;margin:25px 0 14px}.about-copy p:not(.lead){font-size:14px;color:#8995ad;line-height:1.85}.value-grid{display:grid;grid-template-columns:1fr 1fr;gap:11px;align-content:center}.value-grid article{--card-x:50%;--card-y:50%;min-height:120px;padding:17px;border:1px solid rgba(124,153,218,.17);border-radius:14px;background:radial-gradient(circle at var(--card-x) var(--card-y),rgba(112,83,255,.13),transparent 38%),rgba(10,17,37,.62);display:flex;gap:14px;transition:transform .18s ease,border-color .2s ease;transform-style:preserve-3d}.value-grid article:hover{border-color:rgba(119,166,255,.37)}.value-grid i{font-style:normal;color:#65dfff;font-size:20px;text-shadow:0 0 13px rgba(86,215,255,.65)}.value-grid b{display:block;font-size:12px;margin-bottom:6px}.value-grid span{font-size:10px;color:#7e8aa5;line-height:1.55}.japan-card{position:relative;overflow:hidden;border:1px solid rgba(143,162,220,.2);border-radius:16px;background:linear-gradient(#111b47 5%,#ff7c8c 54%,#0e1735 55%);min-height:310px;transform-style:preserve-3d;transition:.18s ease}.sun{position:absolute;width:90px;height:90px;border-radius:50%;background:#ffd3b0;right:22px;top:35px;box-shadow:0 0 42px rgba(255,151,134,.7)}.mountain{position:absolute;bottom:52px;width:0;height:0;border-left:125px solid transparent;border-right:125px solid transparent;border-bottom:170px solid #13264b;filter:drop-shadow(0 -4px 6px rgba(255,255,255,.12))}.m1{left:6px}.m2{right:-60px;border-left-width:100px;border-right-width:100px;border-bottom-width:128px;border-bottom-color:#1a1d4e}.city{position:absolute;left:0;right:0;bottom:41px;height:42px;background:repeating-linear-gradient(90deg,#080d20 0 11px,transparent 11px 16px)}.japan-caption{position:absolute;left:0;right:0;bottom:0;height:48px;display:flex;align-items:center;gap:8px;padding:0 16px;background:rgba(6,10,25,.85);font-size:11px}.japan-caption span{color:#53dcff}
.heading-row{display:flex;justify-content:space-between;align-items:end;margin-bottom:30px}.heading-row h2{font-size:42px}.heading-row p{font-size:12px;color:#7c89a4;margin-bottom:0}.skills-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:11px}.skill-card{--card-x:50%;--card-y:50%;min-height:135px;padding:15px;border:1px solid rgba(115,151,226,.17);border-radius:13px;background:radial-gradient(circle at var(--card-x) var(--card-y),rgba(92,91,255,.15),transparent 45%),rgba(9,15,33,.62);transition:transform .18s ease,border-color .2s ease;transform-style:preserve-3d}.skill-card:hover{border-color:rgba(77,206,255,.35)}.skill-icon{width:40px;height:40px;display:grid;place-items:center;border:1px solid rgba(78,190,255,.25);border-radius:11px;color:#70dcff;background:rgba(36,104,196,.12);font-size:10px;font-weight:900;box-shadow:inset 0 0 20px rgba(64,131,255,.08)}.skill-meta{display:flex;justify-content:space-between;gap:8px;margin-top:16px}.skill-meta b{font-size:11px}.skill-meta span{font-size:8px;color:#687690}.skill-track{height:2px;background:#17223c;margin-top:12px;overflow:hidden;border-radius:99px}.skill-track i{display:block;height:100%;background:linear-gradient(90deg,#29ceff,#8368ff,#e94ed4);box-shadow:0 0 10px #756dff}.skill-note{font-size:9px;color:#596680;margin-top:13px}
.projects-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.project-card{--card-x:50%;--card-y:50%;border:1px solid rgba(126,154,219,.19);border-radius:17px;overflow:hidden;background:rgba(8,14,31,.67);transition:transform .18s ease,border-color .2s ease;transform-style:preserve-3d}.project-card:hover{border-color:rgba(113,181,255,.42)}.project-visual{height:190px;position:relative;overflow:hidden;background:radial-gradient(circle at 65% 40%,rgba(87,84,255,.32),transparent 35%),linear-gradient(145deg,#070b1a,#0e2144 55%,#160d38)}.project-orb{position:absolute;width:120px;height:120px;border-radius:50%;right:22px;top:28px;background:radial-gradient(circle at 35% 32%,#bff7ff 0 2%,#3a9fff 7%,#2950a3 42%,#0b0f31 68%);box-shadow:0 0 40px rgba(59,118,255,.4)}.project-lines{position:absolute;inset:0;background:repeating-linear-gradient(120deg,transparent 0 34px,rgba(87,128,255,.06) 35px,transparent 36px)}.project-visual>span{position:absolute;left:18px;bottom:15px;color:#6d7f9f;font-size:11px;letter-spacing:.14em}.project-body{padding:20px}.project-topline{display:flex;justify-content:space-between;color:#687791;font-size:9px;letter-spacing:.1em}.project-topline b{color:#5ee2ff}.project-body h3{font-size:22px;margin:10px 0 8px}.project-body p{font-size:11px;color:#8996af;line-height:1.72;min-height:58px}.tags{display:flex;gap:6px;flex-wrap:wrap;margin-top:17px}.tags span{font-size:8px;padding:5px 8px;border:1px solid rgba(116,153,230,.2);border-radius:999px;color:#93a4c7;background:#0b1429}
.journey{display:grid;grid-template-columns:.65fr 2fr .65fr;gap:30px;align-items:start}.journey-copy h2{font-size:42px}.journey-copy p{font-size:11px;color:#73819d}.timeline{position:relative;display:grid;grid-template-columns:repeat(3,1fr);gap:20px;padding-top:30px}.timeline:before{content:"";position:absolute;left:0;right:0;top:9px;height:1px;background:linear-gradient(90deg,#4cdcff,#695fff,#ff53c7);box-shadow:0 0 12px rgba(88,120,255,.5)}.timeline article{position:relative}.timeline-dot{position:absolute;width:10px;height:10px;border-radius:50%;background:#94eaff;top:-26px;left:0;box-shadow:0 0 0 3px rgba(80,162,255,.14),0 0 13px #5bcfff}.timeline small{font-size:9px;color:#71809e}.timeline h3{font-size:14px;margin:7px 0 8px}.timeline p{font-size:10px;line-height:1.65;color:#76839f}blockquote{margin:0;padding:20px;border:1px solid rgba(132,103,255,.27);border-radius:14px;background:linear-gradient(145deg,rgba(30,21,70,.46),rgba(12,16,36,.65));font-size:15px;line-height:1.55;color:#d9d9ef;box-shadow:0 0 40px rgba(85,52,255,.08)}blockquote>span{color:#9e8bff;font-size:29px}blockquote small{display:block;margin-top:17px;text-align:right;color:#737f9b;font-size:9px}
